...
{
	it6161_mipi_rx_set_bits(it6161, 0x05, 0x07, 0x00);
}
...
static void mipi_rx_afe_configuration(struct it6161 *it6161, u8 data_id)
{
	...
	dev_info(dev, "afe configuration data_id: 0x%02x", data_id);
...
static void mipi_rx_configuration(struct it6161 *it6161)
{
	...
	it6161_mipi_rx_set_bits(it6161, 0x12, 0x03, 0x00);
	it6161_mipi_rx_set_bits(it6161, 0x18, 0xf7, 0x43);
	it6161_mipi_rx_set_bits(it6161, 0x19, 0xf3, 0x03);
	it6161_mipi_rx_set_bits(it6161, 0x20, 0xf7, 0x03);
	it6161_mipi_rx_set_bits(it6161, 0x21, 0x07, 0x00);
	it6161_mipi_rx_set_bits(it6161, 0x44, 0x38, 0x00);
	it6161_mipi_rx_set_bits(it6161, 0x4B, 0x1f, 0x01);
	it6161_mipi_rx_write(it6161, 0x4C, 0x04);
	it6161_mipi_rx_set_bits(it6161, 0x4D, 0x01, 0x00);
	it6161_mipi_rx_set_bits(it6161, 0x4E, 0x0C, 0x00);
	it6161_mipi_rx_set_bits(it6161, 0x4F, 0x03, 0x01);

	it6161_mipi_rx_write(it6161, 0x27, 0x3E);
	it6161_mipi_rx_set_bits(it6161, 0x70, 0x01, 0x01);
	it6161_mipi_rx_write(it6161, 0x72, 0x04);
	it6161_mipi_rx_write(it6161, 0x73, 0x03);
	it6161_mipi_rx_set_bits(it6161, 0x80, 0x20, 0x00);
	it6161_mipi_rx_write(it6161, 0x21, 0x00);
	it6161_mipi_rx_set_bits(it6161, 0x84, 0x70, 0x00);

	it6161_mipi_rx_set_bits(it6161, 0xA0, 0x01, 0);
	it6161_mipi_rx_set_bits(it6161, 0x70, 0x01, 1);
...
static void hdmi_tx_video_reset(struct it6161 *it6161)
{
	...
	dev_info(dev, "reg04: 0x%02x reg05: 0x%02x reg6: 0x%02x reg07: 0x%02x reg08: 0x%02x reg0e: 0x%02x",
...
static void hdmi_tx_set_capability_from_edid_parse(struct it6161 *it6161, struct edid *edid)
{
	...
	dev_info(dev, "%s mode, monitor %ssupport audio, outputcolormode:%d color_formats:0x%08x color_depth:%d",
				   ...
				   const struct drm_display_mode *adjusted_mode)
{
	...
	dev_info(dev, "    mode " DRM_MODE_FMT "\n", DRM_MODE_ARG(mode));
	dev_info(dev, "adj mode " DRM_MODE_FMT "\n", DRM_MODE_ARG(adjusted_mode));
...
static void it6161_bridge_pre_enable(struct drm_bridge *bridge)
{
	struct it6161 *it6161 = bridge_to_it6161(bridge);
	struct device *dev = &it6161->i2c_hdmi_tx->dev;
	int ret;
	...
	ret = regulator_bulk_enable(IT6161_SUPPLY_NUM, it6161->supplies);
	if (ret)
...
static enum drm_connector_status it6161_bridge_detect(struct drm_bridge *bridge)
{
	...
	dev_info(dev, "hpd:%s", hpd ? "high" : "low");
...
static const struct drm_bridge_funcs it6161_bridge_funcs = {
	...
	.pre_enable = it6161_bridge_pre_enable,
...
static u32 hdmi_tx_calc_rclk(struct it6161 *it6161)
{
	...
	dev_info(dev, "hdmi tx rclk = %d.%d MHz", it6161->hdmi_tx_rclk / 1000,
...
u32 hdmi_tx_calc_pclk(struct it6161 *it6161)
{
	...
	dev_info(dev, "hdmi tx pclk = %d.%d MHz",
...
static void hdmi_tx_get_display_mode(struct it6161 *it6161)
{
	...
	if (interlaced) {
		...
		dev_info(dev, "vdew2nd    = %d\n", vdew2nd);
		dev_info(dev, "vfph2nd    = %d\n", vfph2nd);
		dev_info(dev, "VSyncW2nd  = %d\n", vsyncw2nd);
		dev_info(dev, "vbph2nd    = %d\n", vbph2nd);
		dev_info(dev, "H2ndVRRise = %d\n", H2ndVRRise);
	}

	/* disable video timing read back */
	it6161_hdmi_tx_set_bits(it6161, 0xA8, 0x08, 0x00);

	dev_info(dev, "hdmi tx mode " DRM_MODE_FMT "\n", DRM_MODE_ARG(&it6161->display_mode));
}
...
static void hdmi_tx_setup_pclk_div2(struct it6161 *it6161)
{
	...
	if (HDMI_TX_PCLK_DIV2) {
		dev_info(dev, "PCLK Divided by 2 mode");
...
static void hdmi_tx_setup_csc(struct it6161 *it6161)
{
	 ...
	 */
	dev_info(dev, "hdmi_tx_setup_csc(u8 input_mode = %x,u8 output_mode = %x)\n",
...
static void hdmi_tx_setup_afe(struct it6161 *it6161, u8 level)
{
	...
	dev_info(dev, "setup afe: %s", level ? "high" : "low");
}
			      ...
			      u8 bAudInterface, u32 SampleFreq, u8 ChNum, u8 *pIEC60958ChStat)
{
	...
	case T_AUDIO_HBR:
		...
		break;
	case T_AUDIO_DSD:
		dev_info(dev, "T_AUDIO_DSD\n");
		setHDMITX_DSDAudio();
		break;
	case T_AUDIO_NLPCM:
		dev_info(dev, "T_AUDIO_NLPCM\n");
		pIEC60958ChStat[0] |= 1 << 1;
		setHDMITX_ChStat(it6161, pIEC60958ChStat);
		setHDMITX_NLPCMAudio(bAudInterface);
		break;
	case T_AUDIO_LPCM:
		dev_info(dev, "T_AUDIO_LPCM\n");
		pIEC60958ChStat[0] &= ~(1 << 1);
...
static void mipi_rx_calc_rclk(struct it6161 *it6161)
{
	...
	dev_info(dev, "mipi_rx_rclk = %d,%03d,%03d\n",
...
static void mipi_rx_calc_pclk(struct it6161 *it6161)
{
	...
	dev_info(dev, "it6161->mipi_rx_pclk = %d.%03dMHz",
...
static void mipi_rx_show_mrec(struct it6161 *it6161)
{
	...
	dev_info(dev, "m_hfront_porch    = %d\n", m_hfront_porch);
	dev_info(dev, "m_hsyncw    = %d\n", m_hsyncw);
	dev_info(dev, "m_hback_porch    = %d\n", m_hback_porch);
	dev_info(dev, "m_hactive   = %d\n", m_hactive);
	dev_info(dev, "MHVR2nd = %d\n", MHVR2nd);
	dev_info(dev, "MHBlank  = %d\n", MHBlank);

	dev_info(dev, "m_vfront_porch    = %d\n", m_vfront_porch);
	dev_info(dev, "m_vsyncw    = %d\n", m_vsyncw);
	dev_info(dev, "m_vback_porch   = %d\n", m_vback_porch);
	dev_info(dev, "m_vactive   = %d\n", m_vactive);
	dev_info(dev, "MVFP2nd   = %d\n", MVFP2nd);
	dev_info(dev, "MVTotal = %d\n", MVTotal);
}
...
static void mipi_rx_prec_get_display_mode(struct it6161 *it6161)
{
	...
	dev_info(dev, "mipi rx pmode " DRM_MODE_FMT "\n", DRM_MODE_ARG(&display_mode));
}
...
static void it6161_mipi_rx_interrupt_reg06_process(struct it6161 *it6161, u8 reg06)
{
	...
	if (reg06 & 0x01) {
		...
		dev_info(dev, "PPS M video stable Change Interrupt, %sstable",
					m_video_stable ? "" : "un");

		if (m_video_stable) {
			data_id = it6161_mipi_rx_read(it6161, 0x28);
			dev_info(dev, "mipi receive video format: 0x%02x", data_id);
		...
		dev_info(dev, "PPS P video stable Change Interrupt, %sstable", p_video_stable ? "" : "un");
		if (p_video_stable) {
			dev_info(dev, "PVidStb Change to HIGH");
			mipi_rx_calc_rclk(it6161);
		...
		dev_info(dev, "PPS MHSync error interrupt");
	else if (reg06 & 0x04)
		dev_info(dev, "PPS MHDE Error Interrupt");
	else if (reg06 & 0x08)
		dev_info(dev, "PPS MVSync Error Interrupt");
	else if (reg06 & 0x20)
		dev_info(dev, "PPS PHSync Error Interrupt");
	else if (reg06 & 0x40)
		dev_info(dev, "PPS PHDE Error Interrupt");
	else if (reg06 & 0x80)
		dev_info(dev, "PPS MVDE Error Interrupt");
}
...
static void it6161_mipi_rx_interrupt_reg07_process(struct it6161 *it6161, u8 reg07)
{
	...
	if (reg07 & 0x01)
		dev_info(dev, "PatGen PPGVidStb change interrupt !!!\n");
	else if (reg07 & 0x02)
		dev_info(dev, "PPS Data Byte Error Interrupt !!!\n");
	else if (reg07 & 0x04)
		dev_info(dev, "PPS CMOff Interrupt !!!\n");
	else if (reg07 & 0x08)
		dev_info(dev, "PPS CMOn Interrupt !!!\n");
	else if (reg07 & 0x10)
		dev_info(dev, "PPS ShutDone cmd Interrupt !!!\n");
	else if (reg07 & 0x20)
		dev_info(dev, "PPS TurnOn Interrupt !!!\n");
	else if (reg07 & 0x40) {
		dev_info(dev, "PPS FIFO over read Interrupt !!! tx video statle:%d",
						hdmi_tx_get_video_state(it6161));
		it6161_mipi_rx_set_bits(it6161, 0x07, 0x40, 0x40);
	} else if (reg07 & 0x80) {
		dev_info(dev, "PPS FIFO over write Interrupt !!!\n");
...
static void it6161_mipi_rx_interrupt_reg08_process(struct it6161 *it6161, u8 reg08)
{
	...
	if (reg08 & 0x01)
		dev_info(dev, "ECC 1-bit Error Interrupt !!!\n");
	else if (reg08 & 0x02)
		dev_info(dev, "ECC 2-bit Error Interrupt !!!\n");
	else if (reg08 & 0x04)
		dev_info(dev, "LM FIFO Error Interrupt !!!\n");
	else if (reg08 & 0x08)
		dev_info(dev, "CRC Error Interrupt !!!\n");
	else if (reg08 & 0x10)
		dev_info(dev, "MCLK Off Interrupt !!!\n");
	else if (reg08 & 0x20)
		dev_info(dev, "PPI FIFO OverWrite Interrupt !!!\n");
	else if (reg08 & 0x40) {
		it6161_mipi_rx_set_bits(it6161, 0x0b, 0x40, 0x00);

		if ((it6161_mipi_rx_read(it6161, 0xC1) & 0x03) == 0x03)
			dev_info(dev, "CRC Fail !!!\n");
...
static irqreturn_t it6161_intp_threaded_handler(int unused, void *data)
{
	...
	printk("%s: [MIPI] [REG06 = 0x%x] \n", __func__, mipi_rx_reg06);
	printk("%s: [MIPI] [REG07 = 0x%x] \n", __func__, mipi_rx_reg07);
	printk("%s: [MIPI] [REG08 = 0x%x] \n", __func__, mipi_rx_reg08);
	printk("%s: [MIPI] [REG0D = 0x%x] \n", __func__, mipi_rx_reg0d);
	...
	printk("%s: [HDMI] [REG06 = 0x%x] \n", __func__, hdmi_tx_reg06);
	printk("%s: [HDMI] [REG07 = 0x%x] \n", __func__, hdmi_tx_reg07);
	printk("%s: [HDMI] [REG08 = 0x%x] \n", __func__, hdmi_tx_reg08);
	printk("%s: [HDMI] [REG0E = 0x%x] \n", __func__, hdmi_tx_reg0e);
	printk("%s: [HDMI] [REG0EE = 0x%x] \n", __func__, hdmi_tx_regee);

	if ((mipi_rx_reg06 != 0) || (mipi_rx_reg07 != 0) || (mipi_rx_reg08 != 0)) {
		dev_info(dev, "[MIPI rx ++] reg06: 0x%02x reg07: 0x%02x reg08: 0x%02x reg0d: 0x%02x",
	...
	if ((hdmi_tx_reg06 != 0) || (hdmi_tx_reg07 != 0) || (hdmi_tx_reg08 != 0)) {
		dev_info(dev, "[HDMI tx ++] reg06: 0x%02x reg07: 0x%02x reg08: 0x%02x reg0e: 0x%02x regee: 0x%02x",
					     ...
					     const char *buf, size_t count)
{
	...
	dev_info(dev, "config color space: %s", buf);
	it6161->hdmi_tx_output_color_space &= ~F_MODE_CLRMOD_MASK;
			    ...
			    const struct i2c_device_id *id)
{
	...
	err = it6161_parse_dt(it6161, dev->of_node);
	if (err)
		return err;
